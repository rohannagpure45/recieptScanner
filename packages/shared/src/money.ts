...
export function fromCents(cents: number | string): string {
  ...
  const isNumericString =
    typeof cents === 'string' && /^[+-]?(?:\d*\.\d+|\d+\.?\d*)$/.test(cents.trim());
  if (!(isNumber || isNumericString)) {
    throw new TypeError(
      `fromCents expects a numeric cents value (number or numeric string), got ${String(cents)}`
    );
  }
...
export function allocateRemainder(buckets: number[], targetTotal: number): number[] {
  ...
  const copy = buckets.map((v) => {
    ...
    if (!Number.isFinite(n)) throw new TypeError('allocateRemainder buckets must contain only numbers');
    return Math.max(0, n);
  });
  ...
  const n = copy.length;
  if (diff === 0 || n === 0) return copy.slice();
